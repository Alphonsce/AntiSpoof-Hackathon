...
from data_utils_RawNet2 import (Dataset_dev, Dataset_eval, Dataset_train,
                                genSpoof_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--database_path",
        type=str,
        default="/your/path/to/data/ASVspoof5_database/",
    ...
    parser.add_argument(
        ...
        dest="output_dir",
        type=str,
        default="./ASVspoof5_RawNet2_Baseline",
        help="Dir for Model checkpoint",
    )
    ...
    if args.eval:
        file_eval = genSpoof_list(
            dir_meta=os.path.join(args.database_path + "ASVspoof5.eval.txt"),
            is_train=False,
            is_eval=True,
        )
    ...
    d_label_trn, file_train = genSpoof_list(
        dir_meta=os.path.join(args.database_path + "ASVspoof5.train.metadata.txt"),
        is_train=True,
        is_eval=False,
    )
    ...
    train_set = Dataset_train(
        list_IDs=file_train, labels=d_label_trn, base_dir=args.database_path
    )
    ...
    d_label_dev, file_dev = genSpoof_list(
        dir_meta=os.path.join(args.database_path + "ASVspoof5.dev.metadata.txt"),
        is_train=False,
        is_eval=False,
    )
    ...
    dev_set = Dataset_dev(
        list_IDs=file_dev, labels=d_label_dev, base_dir=args.database_path
    )
    ...
    for epoch in range(num_epochs):
        ...
        print(
            "\n{} - {} - {:.2f} - {:.2f} - {:.2f}".format(
                epoch, running_loss, train_accuracy, val_accuracy, val_err
            )
        )
        logging.info(
