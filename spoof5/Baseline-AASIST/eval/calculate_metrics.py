...
def calculate_minDCF_EER_CLLR(cm_scores_file, output_file, printout=True):
...
def calculate_aDCF_tdcf_tEER(
    cm_scores_file, asv_scores_file, output_file, printout=True
):
    ...
    X_tar = np.array(
        [asv_scores[asv_keys == "target"], cm_scores[asv_keys == "target"]],
        dtype=object,
    )
    X_non = np.array(
        [asv_scores[asv_keys == "nontarget"], cm_scores[asv_keys == "nontarget"]],
        dtype=object,
    )
    X_spf = np.array(
        [asv_scores[asv_keys == "spoof"], cm_scores[asv_keys == "spoof"]], dtype=object
    )
    ...
    eer_cm, frr, far, thresholds = compute_eer(bona_cm, spoof_cm)  # [0]
    minDCF_cm, _ = compute_mindcf(
        frr, far, thresholds, Pspoof, tdcf_cost_model["Cmiss"], tdcf_cost_model["Cfa"]
    )
