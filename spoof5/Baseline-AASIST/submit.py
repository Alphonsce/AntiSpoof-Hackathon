...
from subm_utils.submission_dataset import (EvalDataset,
                                           get_data_for_evaldataset,
                                           get_dataloaders)
...
def main(args):
    ...
    model = get_semaa_model(args.config_path, args.ckpt_path, args.device).to(
        args.device
    )

    produce_submit_file(
        dataloader, model, args.device, args.output_file, need_sigmoid=args.need_sigmoid
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", default="./config/SEMAA_2021.conf")
