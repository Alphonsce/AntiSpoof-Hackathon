...
parser = argparse.ArgumentParser(
    prog="Score-fusion", description="Fuse ASV and CM scores into SASV score", epilog=""
)
...
def get_llrs(data, gm_tar, gm_non, gm_spf):
    ...
    return np.stack(
        [
            ...
            gm_tar.score_samples(data) - gm_spf.score_samples(data),
        ],
        axis=1,
    )
...
def compute_sasv_eer(scores, labels):
    ...
    return util_metric.compute_sasv_eer(
        scores,
        labels,
        args.label_value_target,
        args.label_value_nontarget,
...
if __name__ == "__main__":
    ...
    X_tar_dev = score_label_dev.query("sasv_label == {:f}".format(label_dic["tar"]))[
        ["asv_score", "cm_score"]
    ].to_numpy()
    X_non_dev = score_label_dev.query("sasv_label == {:f}".format(label_dic["non"]))[
        ["asv_score", "cm_score"]
    ].to_numpy()
    X_spf_dev = score_label_dev.query("sasv_label == {:f}".format(label_dic["spf"]))[
        ["asv_score", "cm_score"]
    ].to_numpy()
