...
def print_table(
    ...
    def get_latex_color(data_array, row_idx, col_idx, color_minmax_in):
        ...
        return return_latex_color_cell(
            x, value_min, value_max, colorscale_tmp, colorwrap, color_func
        )
        ...
        for idx, data_row in enumerate(data_array):
            if len(print_format[0]):
                if flag_data_display:
                    ...
                    max_len = max(
                        [
                            len("{num:{form}}".format(num=x, form=print_format[idx]))
                            for x in data_row
                        ]
                    )
    ...
    # latex head
    if pad_dummy_col > 0:
        latex_buffer += r"\begin{tabular}{" + "".join(["c" for x in column_tag + [""]])
    ...
    for row_idx in np.arange(row):
        ...
        for col_idx in np.arange(col):
            ...
            if is_valid_float(data_array[row_idx, col_idx]):
                if len(tmp_print_format):
                    num_str = "{num:{form}}".format(
                        num=data_array[row_idx, col_idx], form=tmp_print_format
                    )
                else:
                    num_str = ""
                latex_color_cell = get_latex_color(
                    data_array, row_idx, col_idx, color_minmax_in
                )
            ...
            row_content_text.append(fill_cell(num_str, col_tag_max_len, col_sep))
