...
def load_cm_scores_keys(cm_scores_file, cm_keys_file, default_index=g_filename_tag):
    ...
    assert set(cm_scores_pd.index) == set(
        cm_keys_pd.index
...
def load_sasv_scores_keys(
    sasv_scores_file, sasv_keys_file, default_index=[g_tar_speaker_tag, g_filename_tag]
):
    """
    input
    -----
    ...
    assert set(sasv_scores_pd.index) == set(
        sasv_keys_pd.index
    ...
    assert (
        sasv_pd.shape[0] == sasv_scores_pd.shape[0]
    ), "Error: SASV score and key incompatible"
    assert (
        sasv_pd.shape[0] == sasv_keys_pd.shape[0]
    ), "Error: SASV score and key incompatible"

    return (
        sasv_pd[g_cm_score_tag].to_numpy(),
...
def load_cm_scores_keys_as_pd(
    cm_scores_file, cm_full_keys_file, default_index=g_filename_tag
):
    """
    input
    -----
