...
def calculate_minDCF_EER_CLLR_actDCF(cm_scores, cm_keys, output_file, printout=True):
    ...
    assert (
        cm_keys.size == cm_scores.size
    ), "Error, unequal length of cm label and score files"
    ...
    # min DCF
    minDCF_cm, _ = compute_mindcf(
        frr, far, thresholds, Pspoof, dcf_cost_model["Cmiss"], dcf_cost_model["Cfa"]
    )
    # actual DCF
    actDCF, _ = compute_actDCF(
        bona_cm, spoof_cm, Pspoof, dcf_cost_model["Cmiss"], dcf_cost_model["Cfa"]
    )
    ...
    if printout:
        with open(output_file, "w") as f_res:
            f_res.write("\nCM SYSTEM\n")
            f_res.write(
                "\tmin DCF \t\t= {} " "(min DCF for countermeasure)\n".format(minDCF_cm)
            )
            f_res.write(
                "\tEER\t\t= {:8.9f} % "
...
def calculate_aDCF_tdcf_tEER(
    ...
    adcf = a_dcf._calculate_a_dcf(tar_sasv, nontar_sasv, spoof_sasv, cost_adcf, False)[
        "min_a_dcf"
    ]
    ...
    # asv-cm score pairs
    X_tar = np.array(
        [asv_scores[asv_keys == util.g_asv_tar], cm_scores[asv_keys == util.g_asv_tar]],
        dtype=object,
    )
    X_non = np.array(
        [asv_scores[asv_keys == util.g_asv_non], cm_scores[asv_keys == util.g_asv_non]],
        dtype=object,
    )
    X_spf = np.array(
        [asv_scores[asv_keys == util.g_asv_spf], cm_scores[asv_keys == util.g_asv_spf]],
        dtype=object,
    )
    ###
