...
class ASVspoof2019LA(data.Dataset):
    ...
    """
    def __init__(self, rawboost_args, sys_config=config.SysConfig(), exp_config=config.ExpConfig()):
    ...
    def __getitem__(self, index: Any) -> Any:
        ...
        utter, sr = torchaudio.load(utter)
        utter = self.adjustDuration(utter)
        utter = torch.tensor(process_Rawboost_feature(utter, sr, self.rawboost_args, self.rawboost_args.algo)).to(utter)
