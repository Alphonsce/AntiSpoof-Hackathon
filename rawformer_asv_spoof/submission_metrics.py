...
from submission_utils import progressbar
...
def produce_evaluation_file(
    ...
    current_loss = 0
    # inference
    for batch_x, utt_id, batch_y in progressbar(
        data_loader, prefix="computing cm score"
    ):
...
@torch.inference_mode()
def produce_submit_file(data_loader, model, device, save_path):
    """
    Create file, that need to give in function calculcate_t-DCF_EER
    args:
        data_loader: loader, that gives batch to model
