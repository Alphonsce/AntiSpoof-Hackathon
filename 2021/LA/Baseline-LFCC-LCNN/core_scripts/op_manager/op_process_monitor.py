...
class Monitor:
        ...
        try:
            ...
            if self.epoch_num == state_dic["epoch_num"]:
                ...
                self.loss_mat = np.resize(
                    self.loss_mat, [self.epoch_num, self.seq_num, tmp_loss_mat.shape[2]]
                )
    ...
    def log_loss(self, loss, loss_flag, time_cost, seq_info, seq_idx, epoch_idx):
        """Log down the loss"""
        self.seq_names[seq_idx] = seq_info
        if self.loss_mat.shape[-1] != len(loss):
            self.loss_mat = np.resize(
                self.loss_mat,
                [self.loss_mat.shape[0], self.loss_mat.shape[1], len(loss)],
            )
