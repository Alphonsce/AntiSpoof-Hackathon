...
def f_run_one_epoch(
    ...
    for data_idx, (data_in, data_tar, data_info, idx_orig) in enumerate(data_loader):
        ...
        #   los_vals: [loss.item()]
        #   loss_flags: [True]
        loss, loss_values, loss_flags = nii_nn_tools.f_process_loss(loss_computed)
        ...
        for idx, data_seq_info in enumerate(data_info):
            ...
            if args.verbose == 1:
                monitor.print_error_for_batch(
                    data_idx * batchsize + idx, idx_orig.numpy()[idx], epoch_idx
                )
            #
...
def f_train_wrapper(
    ...
    if checkpoint is not None:
        if type(checkpoint) is dict:
            ...
            if (
                cp_names.optimizer in checkpoint
                and not args.ignore_optimizer_statistics_in_trained_model
            ):
            ...
            if not args.ignore_training_history_in_trained_model:
                ...
                if (
                    cp_names.lr_scheduler in checkpoint
                    and checkpoint[cp_names.lr_scheduler]
                    and lr_scheduler.f_valid()
                ):
                    lr_scheduler.f_load_state_dict(checkpoint[cp_names.lr_scheduler])
            ...
            # only model status
            pt_model.load_state_dict(
                nii_nn_tools.f_state_dict_wrapper(checkpoint, flag_multi_device)
            )
    ...
    if hasattr(pt_model, "g_pretrained_model_path") and hasattr(
        ...
        nii_nn_tools.f_load_pretrained_model_partially(
            pt_model,
            pt_model.g_pretrained_model_path,
            pt_model.g_pretrained_model_prefix,
        )
    ...
    for epoch_idx in range(start_epoch, epoch_num):
        ...
        if not args.not_save_each_epoch:
            ...
            if args.verbose == 1:
                ...
                nii_display.f_eprint("Save {:s}".format(tmp_model_name), flush=True)

        # Early stopping
        #  note: if LR scheduler is used, early stopping will be
        #  disabled
        if (
            lr_scheduler.f_allow_early_stopping()
            and monitor_val is not None
            and monitor_val.should_early_stop(no_best_epoch_num)
        ):
...
def f_inference_wrapper(args, pt_model, device, test_dataset_wrapper, checkpoint):
    ...
    with torch.no_grad():
        for _, (data_in, data_tar, data_info, idx_orig) in enumerate(test_data_loader):
            ...
            if data_gen is None:
                ...
                for idx, seq_info in enumerate(data_info):
                    _ = nii_op_display_tk.print_gen_info(seq_info, time_cost)
                    test_dataset_wrapper.putitem(
                        data_gen_np[idx : idx + 1], args.output_dir, seq_info
                    )
