...
def main(config):
    ...
    if config["model"] == "ResCapsGuard":
        loss_fn = CapsuleLoss(
            gpu_id=wandb.config["gpu_id"], weight=torch.FloatTensor([0.1, 0.9])
        )
    elif config["model"] == "Res2TCNGuard":
        loss_fn = nn.CrossEntropyLoss(
            weight=torch.FloatTensor([0.1, 0.9]).to(config["device"])
        )
    ...
    for epoch in range(config["epoches"]):
        ...
        eer, tdcf = calculate_eer_tdcf(
            cm_scores_file=config["produced_file"],
