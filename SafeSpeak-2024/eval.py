...
def main(args, cfg):
    ...
    if cfg["model"] == "ResCapsGuard":
        loss_fn = CapsuleLoss(
            gpu_id=cfg["gpu_id"], weight=torch.FloatTensor([0.1, 0.9])
        )
    elif cfg["model"] == "Res2TCNGuard":
        loss_fn = nn.CrossEntropyLoss(
            weight=torch.FloatTensor([0.1, 0.9]).to(cfg["device"])
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="configs/config_rescapsguard.json"
    )
    parser.add_argument("--eval_label_path", type=str)
    parser.add_argument("--eval_path_flac", type=str)
