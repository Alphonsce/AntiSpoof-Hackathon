...
class SincConv(nn.Module):
    ...
    def __init__(
        ...
        )  # learnable f1 from the paper

        # filter frequency band (out_channels, 1)
        self.band_hz_ = nn.Parameter(
            torch.Tensor(np.diff(hz)).view(-1, 1)
        )  # learnable f2 (f2 = f1+diff) from the paper
    ...
    def forward(self, waveforms):
        ...
        low = self.min_low_hz + torch.abs(
            self.low_hz_
        )  # eq. (5) + make sure low >= min_low_hz

        high = torch.clamp(
            low + self.min_band_hz + torch.abs(self.band_hz_),
            self.min_low_hz,
            self.sample_rate / 2,
        )  # eq. (6) + make sure band has length >= min_band_hz
        ...
        band_pass_center = 2 * band.view(
            -1, 1
        )  # g[0] = 2 * (f2 - f1) = 2 * band, w[0] = 1
...
class Res2Block(nn.Module):
    def __init__(self, nb_filts, first=False, nums=4):
        ...
        for i in range(self.nums):
            convs.append(
                nn.Conv2d(
                    in_channels=(nb_filts[1] // self.nums),
