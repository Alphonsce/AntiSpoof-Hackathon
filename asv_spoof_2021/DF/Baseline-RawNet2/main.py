...
from data_utils import (Dataset_ASVspoof2019_train, Dataset_ASVspoof2021_eval,
                        genSpoof_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--database_path",
        type=str,
        default="/your/path/to/data/ASVspoof_database/",
        help="Change this to user's full directory address of DF database. We assume that all three ASVspoof 2019 LA train, LA dev and ASVspoof2021 DF eval data folders are in the same database_path directory.",
    )
    """
    % database_path/
    %   |- DF
    ...
    parser.add_argument(
        "--protocols_path",
        type=str,
        default="/your/path/to/protocols/ASVspoof_database/",
    ...
    # Auxiliary arguments
    parser.add_argument(
        "--track", type=str, default="DF", choices=["LA", "PA", "DF"], help="LA/PA/DF"
    )
    parser.add_argument(
        "--eval_output",
        type=str,
    ...
    d_label_trn, file_train = genSpoof_list(
        dir_meta=os.path.join(
            args.protocols_path
            + "{}_cm_protocols/ASVspoof2019.LA.cm.train.trn.txt".format(prefix)
        ),
        is_train=True,
        is_eval=False,
    )
    ...
    d_label_dev, file_dev = genSpoof_list(
        dir_meta=os.path.join(
            args.protocols_path
            + "{}_cm_protocols/ASVspoof2019.LA.cm.dev.trl.txt".format(prefix)
        ),
        is_train=False,
        is_eval=False,
    )
