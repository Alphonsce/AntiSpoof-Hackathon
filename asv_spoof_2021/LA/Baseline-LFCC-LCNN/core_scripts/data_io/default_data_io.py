...
class NIIDataSet(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if len(self.m_input_dirs) != len(self.m_input_exts) or len(
            self.m_input_dirs
        ) != len(self.m_input_dims):
            nii_warn.f_print("Input dirs, exts, dims, unequal length", "error")
        ...
        if len(self.m_output_dims) != len(self.m_output_exts) or (
            self.m_output_dirs and len(self.m_output_dirs) != len(self.m_output_exts)
        ):
        ...
        if np.any(np.array(self.m_output_reso) < 0):
            nii_warn.f_print("NIIDataSet not support negative reso", "error", end="")
        ...
        self.m_ms_input_path = get_name(
            stats_path, self.m_set_name, nii_dconf.mean_std_i_file
        )
        self.m_ms_output_path = get_name(
            stats_path, self.m_set_name, nii_dconf.mean_std_o_file
        )
        self.m_data_len_path = get_name(
            stats_path, self.m_set_name, nii_dconf.data_len_file
        )

        # initialize data length and mean /std, read prepared data stats
        flag_cal_len = self.f_init_data_len_stats(self.m_data_len_path)
        flag_cal_mean_std = self.f_init_mean_std(
            self.m_ms_input_path, self.m_ms_output_path
        )
    ...
    def __getitem__(self, idx):
        ...
        for t_dir, t_ext, t_dim, t_res in zip(
            self.m_input_dirs, self.m_input_exts, self.m_input_dims, self.m_input_reso
        ):
            e_dim = s_dim + t_dim
        ...
        if self.m_output_dirs:
            ...
            s_dim = 0
            e_dim = 0
            for t_dir, t_ext, t_dim in zip(
                self.m_output_dirs, self.m_output_exts, self.m_output_dims
            ):
                e_dim = s_dim + t_dim
                # get file path and load data
                file_path = nii_str_tk.f_realpath(t_dir, file_name, t_ext)
        ...
        if self.m_opt_wav_handler > 0:
            ...
            if len(self.m_input_exts) == 1 and self.m_input_exts[0][-3:] == "wav":
                in_data_n = nii_wav_tk.silence_handler(
                    in_data[:, 0], self.m_wav_sr, flag_output=self.m_opt_wav_handler
                )
            ...
            if len(self.m_output_exts) == 1 and self.m_output_exts[0][-3:] == "wav":
                out_data_n = nii_wav_tk.silence_handler(
                    out_data[:, 0], self.m_wav_sr, flag_output=self.m_opt_wav_handler
                )
    ...
    def f_check_file_list(self):
        ...
        if not isinstance(self.m_file_list, list):
            if isinstance(self.m_file_list, str) and os.path.isfile(self.m_file_list):
                ...
                self.m_file_list = nii_list_tools.read_list_from_text(self.m_file_list)
            else:
        ...
        for tmp_d, tmp_e in zip(self.m_input_dirs, self.m_input_exts):
            ...
            self.m_file_list = nii_list_tools.common_members(tmp_list, self.m_file_list)
        ...
        if len(self.m_file_list) < 1:
            ...
            nii_warn.f_print("They should contain all files in file list", "error")
            nii_warn.f_print(
                "Please also check filename extentions %s" % (str(self.m_input_exts)),
                "error",
            )
        ...
        if self.m_output_dirs:
            ...
            if len(self.m_file_list) < 1:
                ...
                nii_warn.f_print("They should contain all files in file list", "error")
                nii_warn.f_print(
                    "Please also check filename extentions %s"
                    % (str(self.m_output_exts)),
                    "error",
                )
                nii_warn.f_print("They should be correctly specified", "error")
    ...
    def f_check_specific_data(self, file_name):
        ...
        for t_dir, t_ext, t_dim, t_res in zip(tmp_dirs, tmp_exts, tmp_dims, tmp_reso):
            ...
            if not nii_io_tk.file_exist(file_path):
                ...
                print(
                    "%s, length %d, dim %d, reso: %d" % (file_path, t_len, t_dim, t_res)
                )
        return
    ...
    def f_log_data_len(self, file_name, t_len, t_reso):
        ...
        if valid_flag:
            ...
            if t_len == 1:
                ...
                pass
            elif self.f_valid_len(
                self.m_data_length[file_name], tmp_len, nii_dconf.data_seq_min_length
            ):
    ...
    def f_precheck_data_length(self):
        ...
        if (
            not self.m_data_length
            and not self.m_output_dirs
            and all([x < 0 for x in self.m_input_reso])
        ):
            ...
            if self.m_truncate_seq is not None:
                while length_remain > 0:
                    ...
                    seq_info = nii_seqinfo.SeqInfo(
                        seg_length, file_name, seg_idx, start_pos, info_idx
                    )
                ...
                info_idx = len(self.m_seq_info)
                seq_info = nii_seqinfo.SeqInfo(
                    length_remain, file_name, seg_idx, start_pos, info_idx
                )
    ...
    def f_init_mean_std(self, ms_input_path, ms_output_path):
        ...
        if os.path.isfile(ms_input_path) and os.path.isfile(ms_output_path):
            ...
            if ms_input.shape[0] != (self.m_input_all_dim * 2) or ms_output.shape[
                0
            ] != (self.m_output_all_dim * 2):
                if ms_input.shape[0] != (self.m_input_all_dim * 2):
                    nii_warn.f_print("%s incompatible" % (ms_input_path), "warning")
                if ms_output.shape[0] != (self.m_output_all_dim * 2):
                    nii_warn.f_print("%s incompatible" % (ms_output_path), "warning")
                ...
                nii_warn.f_print(
                    "Load mean/std from %s and %s" % (ms_input_path, ms_output_path)
                )
                flag = False
        return flag
    ...
    def f_calculate_stats(self, flag_cal_data_len, flag_cal_mean_std):
        ...
        # loop over each input/output feature type
        for t_dir, t_ext, t_dim, t_reso, t_norm in zip(
            tmp_dirs, tmp_exts, tmp_dims, tmp_reso, tmp_norm
        ):
        ...
        if flag_cal_mean_std:
            self.f_save_mean_std(self.m_ms_input_path, self.m_ms_output_path)
        # done
        return
...
class NIIDataSetLoader:
    ...
    def __init__(
        ...
        """
        nii_warn.f_print_w_date("Loading dataset %s" % (dataset_name), level="h")

        # create torch.util.data.DataSet
        self.m_dataset = NIIDataSet(
            dataset_name,
            file_list,
            input_dirs,
            input_exts,
            ...
            if tmp_params["sampler"] == nii_sampler_fn.g_str_sampler_bsbl:
                if "batch_size" in tmp_params:
                    ...
                    tmp_sampler = nii_sampler_fn.SamplerBlockShuffleByLen(
                        self.m_dataset.f_get_seq_len_list(), tmp_params["batch_size"]
                    )
