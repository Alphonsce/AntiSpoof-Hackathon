...
class SignalFraming(torch_nn.Conv1d):
    ...
    def __init__(self, w_len, h_len, w_type="Hamming"):
        super(SignalFraming, self).__init__(
            1, w_len, w_len, stride=h_len, padding=0, dilation=1, groups=1, bias=False
        )
        self.m_wlen = w_len
        self.m_wtype = w_type
        self.m_hlen = h_len
    ...
    def forward(self, signal):
        ...
        signal_pad = torch_nn_func.pad(
            signal.permute(0, 2, 1), (self.m_pad_len_l, self.m_pad_len_r)
        )
...
class MaxPool1dStride(torch_nn.MaxPool1d):
    ...
    def __init__(self, kernel_s, stride, dilation_s=1):
        super(MaxPool1dStride, self).__init__(kernel_s, stride, 0, dilation_s)
    ...
    def forward(self, data):
        ...
        x = (
            torch_nn_func.pad(
                data.permute(0, 2, 1).unsqueeze(2), (self.pad_le, self.pad_ri, 0, 0)
            )
            .squeeze(2)
            .contiguous()
        )
...
class AvePool1dStride(torch_nn.AvgPool1d):
    ...
    def __init__(self, kernel_s, stride):
        super(AvePool1dStride, self).__init__(kernel_s, stride, 0)
...
class SelfWeightedPooling(torch_nn.Module):
    ...
    def _forward(self, inputs):
        ...
        if self.num_head == 1:
            ...
            weighted = torch.bmm(
                inputs.view(-1, feat_dim, 1), attentions.view(-1, 1, self.num_head)
            )
...
class Conv1dForARModel(Conv1dKeepLength):
    ...
    def __init__(
        self, input_dim, output_dim, dilation_s, kernel_s, bias=True, tanh=True
    ):
...
class AdjustTemporalResoIO(torch_nn.Module):
    def __init__(self, list_reso, target_reso, list_dims):
        ...
        if any(
            [x % self.target_reso != 0 for x in self.list_reso if x > self.target_reso]
        ):
        ...
        if any(
            [self.target_reso % x != 0 for x in self.list_reso if x < self.target_reso]
        ):
            print("Resolution " + str(list_reso) + " incompatible")
            print(" with target resolution {:d}".format(self.target_reso))
            sys.exit(1)
    ...
    def forward(self, tensor_list):
        ...
        output_tensor_list = []
        for in_tensor, dim_fac, reso_fac, l_up in zip(
            tensor_list, self.dim_change, self.reso_change, self.l_upsampler
        ):
            ...
            if reso_fac == 1:
                ...
                if expected_len == 0:
                    ...
                    output_tensor_list.append(
                        torch.reshape(in_tensor[:, 0:1, :], (batchsize, 1, -1))
                    )
                else:
                    # make sure that
                    output_tensor_list.append(
                        torch.reshape(
                            in_tensor[:, 0:trim_length, :],
...
class LSTMZoneOut(torch_nn.Module):
    ...
    def forward(self, x):
        ...
        # output tensor
        y = torch.zeros(
            [batchsize, length, self.out_dim], device=x.device, dtype=x.dtype
        )
        ...
        if self.flag_bi:
            # for bi-directional
            hid1 = torch.zeros(
                [batchsize, self.out_dim // 2], device=x.device, dtype=x.dtype
            )
            ...
            # for uni-directional
            hid1 = torch.zeros(
                [batchsize, self.out_dim], device=x.device, dtype=x.dtype
            )
            cell1 = torch.zeros_like(hid1)
