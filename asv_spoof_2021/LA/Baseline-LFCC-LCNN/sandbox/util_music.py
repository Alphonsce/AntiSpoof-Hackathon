...
class HzCentConverter(torch_nn.Module):
    ...
    def __init__(
        ...
        self.m_dis_cent = torch_nn.Parameter(
            torch.linspace(
                self.m_1st_cent.numpy()[0], self.m_last_cent.numpy()[0], self.m_bins
            ),
            requires_grad=False,
        )
    ...
    def recover_f0(self, bin_mat):
        ...
        if not self.m_viterbi_decode:
            # normal sum
            cent = torch.sum(bin_mat * self.m_dis_cent, axis=2) / torch.sum(
                bin_mat, axis=2
            )
            return self.cent2hz(cent)
        else:
            ...
            with torch.no_grad():
                ...
                prob_u = torch.ones_like(tmp_bin_mat) - torch.mean(
                    tmp_bin_mat, axis=2, keepdim=True
                )
                ...
                cent = torch.sum(prob_m * self.m_dis_cent, axis=2) / torch.sum(
                    prob_m, axis=2
                )
    ...
    def f0_probmat_postprocessing(self, f0_prob_mat):
        ...
        # observation probablity for unvoiced states
        prob_u = torch.ones_like(f0_prob_mat) - torch.mean(
            f0_prob_mat, axis=2, keepdim=True
        )
