...
def compute_tDCF(
    ...
    if Pfa_spoof_asv is None:
        sys.exit(
            "ERROR: you should provide false alarm rate of spoof tests against your ASV system."
        )
    ...
    if print_cost:
        ...
        print(
            "   Cfa          = {:8.5f} (Cost of tandem system falsely accepting a nontarget)".format(
                cost_model["Cfa"]
            )
        )
        print(
            "   Cmiss        = {:8.5f} (Cost of tandem system falsely rejecting target speaker)".format(
                cost_model["Cmiss"]
            )
        )
        print(
            "   Cfa_spoof    = {:8.5f} (Cost of tandem sysmte falsely accepting spoof)".format(
                cost_model["Cfa_spoof"]
            )
        )
        print(
...
def ASVspoof2019_evaluate(
    bonafide_cm_scores,
    bonafide_cm_file_names,
    spoof_cm_scores,
    spoof_cm_file_names,
    verbose=False,
    ...
    spoof_type_dic = {
        protocol_data[x][1]: protocol_data[x][3] for x in range(protocol_data.shape[0])
    }
    ...
    for idx, spoof_attack_type in enumerate(spoof_attack_types):
        tmp_spoof_scores = [
            spoof_cm_scores[x]
            for x, y in enumerate(spoof_cm_file_names)
    ...
    for idx in range(len(spoof_attack_types)):
        if verbose and eer_cm[idx] > -1:
            print(
                "{:s}\tmin-tDCF: {:2.5f}\tEER: {:2.3f}%\t Thre:{:f}".format(
                    spoof_attack_types[idx],
                    min_tDCF[idx],
                    eer_cm[idx] * 100,
                    eer_threshold[idx],
                )
    ...
    decomposed_spoof_scores = [
        decomposed_spoof_scores[x] for x, y in enumerate(min_tDCF) if y > -1
    ]
    spoof_attack_types = [
        spoof_attack_types[x] for x, y in enumerate(min_tDCF) if y > -1
    ]
...
def ASVspoof2019_decomposed_results(
    score_file_path, flag_return_results=False, flag_verbose=True
):
...
def ASVspoofNNN_decomposed_results(
    score_file_path,
...
def read_asv_txt_file(file_path):
    data = np.genfromtxt(
        file_path,
        dtype=[("class", "U10"), ("type", "U10"), ("score", "f4")],
        delimiter=" ",
    )
...
def read_cm_txt_file(file_path):
    data = np.genfromtxt(
        file_path,
        dtype=[("class", "U10"), ("type", "U10"), ("flag", "U10"), ("score", "f4")],
        delimiter=" ",
    )
