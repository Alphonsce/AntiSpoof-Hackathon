...
def nsgcqwin(*args):
    ...
    if not np.all(tmpIdx == 0):
        ...
        warnings.warn(
            "fmin set to" + str(fftres * math.floor(fbas[0] / fftres), 6) + " Hz!"
        )
    ...
    posit[: Lfbas + 2] = np.array(
        [math.floor(fbas_num) for fbas_num in list(fbas[: Lfbas + 2])]
    )
    # print(len(posit))
    posit[Lfbas + 2 :] = np.array(
        [math.ceil(fbas_num) for fbas_num in list(fbas[Lfbas + 2 :])]
    )
    # print(posit)
    # print(len(posit))
    ...
    if fractional:
        ary = (
            (
                np.array(list(range(math.ceil(M / 2) + 1)))
                + np.array(list(range(-math.floor(M / 2), 0)))
            )
            .conj()
            .T
        )
