...
def nsgtf_real(*args):
    ...
    for ii in range(N + 1):
        ...
        win_range = (
            posit[ii] + np.arange(int(-np.floor(Lg[ii] / 2)), int(np.ceil(Lg[ii] / 2)))
        ) % (Ls + fill)
        ...
        if M[ii] < Lg[ii]:  # if the number of frequency channels is too small,
            ...
            idx_list = list(range(end - np.floor(Lg[ii] / 2) + 1 - 1, end)) + list(
                range(0, np.ceil(Lg[ii] / 2))
            )
            ...
            idx_list = list(range(int(end - np.floor(Lg[ii] / 2) + 1 - 1), end)) + list(
