...
def parse_argument():
    ...
    parser.add_argument(
        "--recompute-c012", action="store_true", default=False, help=mes
    )
            ...
            for factor_name_2, factor_list_2, factor_type_2 in zip(
                ...
                for _, factor_2 in enumerate(factor_list_2):
                    ...
                    if factor_2 == pooled_tag:
                        ...
                        qr_tar_fac2 = ""
                        qr_ntar_fac2 = ""
                        qr_spoof_fac2 = ' and {:s} == "{:s}"'.format(
                            factor_name_2, factor_2
                        )
                    elif factor_type_2 == config.g_factor_type_bonafide:
                        qr_tar_fac2 = ' and {:s} == "{:s}"'.format(
                            factor_name_2, factor_2
                        )
                        qr_ntar_fac2 = ' and {:s} == "{:s}"'.format(
                            factor_name_2, factor_2
                        )
                        qr_spoof_fac2 = ""
                    else:
                    ...
                    qr_tar = (
                        'label == "{:s}"'.format(target_tag) + qr_tar_fac1 + qr_tar_fac2
                    )
                    qr_ntar = (
                        'label == "{:s}"'.format(nontarget_tag)
                        + qr_ntar_fac1
                        + qr_ntar_fac2
                    )
                    qr_spoof = (
                        'label == "{:s}"'.format(spoofed_tag)
                        + qr_spoof_fac1
                        + qr_spoof_fac2
                    )
...
def evaluation_API(
    ...
    protocol_cm_pd = pd_tools.load_protocol(
        protocol_cm_file, names=config_buf.p_names, index_col=config_buf.index_col
    )
    # load score file
    score_cm_pd = pd_tools.load_score(
        cm_score_file, config_buf.s_names, index_col=config_buf.index_col
    )
    ...
    if config_buf.flag_tDCF and flag_recompute_c012:
        ...
        print("===============\nCompute C012 coef\n===============")
        protocol_asv_pd = pd_tools.load_protocol(
            protocol_asv_file, names=config_buf.p_names_asv
        )
        # load score file
        asv_score_pd = pd_tools.load_score(asv_score_file, config_buf.s_names_asv)
        # merge score and protocol into a single dataFrame
