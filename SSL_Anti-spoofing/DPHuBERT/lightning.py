    ...
    def get_lr(self):
        if self._step_count <= self.warmup_updates:
            return [
                ...
                * (
                    self.init_lr_scale
                    + self._step_count / self.warmup_updates * (1 - self.init_lr_scale)
                )
                for base_lr in self.base_lrs
            ]
        elif (
            self.warmup_updates
            < self._step_count
            <= (self.warmup_updates + self.hold_updates)
        ):
            return list(self.base_lrs)
        elif self._step_count <= (
            self.warmup_updates + self.hold_updates + self.decay_updates
        ):
            return [
                base_lr
                * math.exp(
...
class DistillLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        loss = (
            self.l2_weight * loss_mse
            + self.l1_weight * loss_l1
            + self.cos_weight * loss_cos
        )
...
class DistillModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        main_params = [
            p for n, p in self.student_model.named_parameters() if "log_alpha" not in n
        ]
    ...
    def _step(self, batch, batch_idx, mode):
        ...
        with torch.no_grad():
            teacher_hiddens, teacher_lengths = self.teacher_model.extract_features(
                waveforms, lengths
            )
        ...
        student_hiddens, student_lengths = self.student_model.extract_features(
            waveforms, lengths
        )
        new_student_hiddens = []
        ...
        if self.use_reg:
            ...
            cur_expected_sparsity = (
                1.0 - self.student_model.get_num_params() / self.original_num_params
            )
            loss_reg = (
                self.lambda1 * (cur_expected_sparsity - cur_target_sparsity)
    ...
    def train_dataloader(self):
        ...
        dataloader = DataLoader(
            ...
            collate_fn=CollateFnAudio(
                pad=False, rand_crop=True
            ),  # crop to the min length in a mini-batch
