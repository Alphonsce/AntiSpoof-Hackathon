...
class GuidedCrossEntAccCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        is_dual_input = (
            True
            if net_input["src_tokens"] is not None
            and net_input.get("src_txt_tokens") is not None
            else False
        )
        ...
        if is_dual_input:
            ...
            speech_loss, speech_nll_loss, speech_correct, speech_total = (
                self.guide_loss_and_acc(
                    model, lprobs_spch, lprobs_text, target, reduce=(reduction == "sum")
                )
            )
            text_loss, text_nll_loss, text_correct, text_total = (
                self.compute_loss_and_acc(
            ...
            loss, nll_loss, correct, total = self.compute_loss_and_acc(
                model, lprobs, target, reduction=reduction
            )
    ...
    def guide_loss_and_acc(self, model, lprobs, lprobs_teacher, target, reduce=True):
        ...
        if self.alpha == 0.0 or model.num_updates < self.disable_update_num:
            return self.compute_loss_and_acc(
                model, lprobs, target, reduction=("sum" if reduce else "none")
            )
        if not lprobs.batch_first:
            lprobs = lprobs.transpose(0, 1)
        ...
        lprobs_teacher = lprobs_teacher.view(
            -1, lprobs_teacher.size(-1)
        ).float()  # -> (B x T) x C
        target = target.view(-1)
        loss = F.nll_loss(
            lprobs,
        ...
        nll_loss = loss
        probs_teacher = lprobs_teacher.exp().masked_fill_(
            target.unsqueeze(-1).eq(self.padding_idx), 0
        )
        probs_teacher = probs_teacher.detach()
        guide_loss = (
            -(probs_teacher * lprobs).sum()
            if reduce
            else -(probs_teacher * lprobs).sum(-1, keepdim=True)
        )
    ...
    def aggregate_logging_outputs(logging_outputs):
        ...
        speech_nll_loss_sum = sum(
            log.get("speech_nll_loss", 0) for log in logging_outputs
        )
        speech_attn_loss_sum = sum(
            log.get("speech_attn_loss", 0) for log in logging_outputs
        )
        sample_size_speech = sum(
            log.get("sample_size_speech_cost", 0) for log in logging_outputs
        )
