...
def f_run_one_epoch_GAN(
    ...
    for data_idx, (data_in, data_tar, data_info, idx_orig) in enumerate(data_loader):
        ...
        if hasattr(loss_wrapper, "compute_feat_match"):
            errG_feat = loss_wrapper.compute_feat_match(d_out_real, d_out_fake_for_G)
        else:
...
def f_train_wrapper_GAN(
    ...
    nii_nn_tools.f_model_show(pt_model_D, do_model_def_check=False, model_type="GAN")
    ...
    if checkpoint_G is not None or checkpoint_D is not None:
        for checkpoint, optimizer, pt_model, model_name in zip(
            [checkpoint_G, checkpoint_D],
            [optimizer_G, optimizer_D],
            [pt_model_G, pt_model_D],
    ...
    for epoch_idx in range(start_epoch, epoch_num):
        ...
        train_log += nii_op_display_tk.print_train_info(
            ...
            loss_val,
            flag_new_best,
            optimizer_G_wrapper.get_lr_info(),
        )
        ...
        if not args.not_save_each_epoch:
            ...
            for pt_model, optimizer, model_tag in zip(
                [pt_model_G, pt_model_D], [optimizer_G, optimizer_D], model_tags
            ):
    ...
    for model_tag in model_tags:
        nii_display.f_print(
            "{}".format(nii_nn_tools.f_save_trained_name(args, model_tag))
        )
    return
