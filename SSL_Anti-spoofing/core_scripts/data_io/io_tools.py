...
def f_write_htk(
    data, targetfile, sampPeriod=50000, sampKind=9, data_format="f4", end="l"
):
    """
    ...
    temp_data = np.array([nSamples, sampPeriod], dtype=np.dtype(data_format))
    temp_data.tofile(f, "")
...
def wrapper_data_load_with_cache(
    file_path,
    method_data_load,
    cache_dir="__cache",
