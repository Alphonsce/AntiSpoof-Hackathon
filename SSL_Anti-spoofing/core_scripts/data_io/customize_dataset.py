...
class merge_loader:
    ...
    def __init__(self, datasets):
        ...
        self.m_idx_shift = np.cumsum([0] + [x.get_seq_num() for x in self.m_datasets])
        return
    ...
    def adjust_utt_idx(self, data_tuple, dataset_idx):
        ...
        """
        return (
            self.m_datasets[dataset_idx]
            .get_dataset()
            .f_adjust_idx(data_tuple, self.m_idx_shift[dataset_idx])
        )
    ...
    def __next__(self):
        ...
        try:
            ...
            for dataset_idx, dataloader in enumerate(self.m_loader_iter):
                data_list.append(self.adjust_utt_idx(next(dataloader), dataset_idx))
...
class NII_MergeDataSetLoader:
    ...
    def __init__(
        ...
        if (
            type(list_input_dirs[0]) is list
            and type(list_output_dirs[0]) is list
            and type(list_file_list) is list
            and len(list_input_dirs) == len(list_output_dirs)
            and len(list_input_dirs) == len(list_file_list)
        ):
            pass
        else:
            ...
            mes += "{:s}\n{:s}\n{:s}".format(
                str(list_input_dirs), str(list_output_dirs), str(list_file_list)
            )
            nii_warn.f_die(mes)
        ...
        if type(dataset_name) is list:
            ...
            tmp_dnames = [
                dataset_name + "_sub_{:d}".format(idx)
                for idx in np.arange(len(list_input_dirs))
            ]

        # create individual datasets
        lst_dset = []
        for sub_input_dirs, sub_output_dirs, sub_file_list, tmp_name in zip(
            list_input_dirs, list_output_dirs, list_file_list, tmp_dnames
        ):
                ...
                if tmp_params["sampler"] == nii_sampler_fn.g_str_sampler_bsbl:
                    if "batch_size" in tmp_params:
                        ...
                        tmp_sampler = nii_sampler_fn.SamplerBlockShuffleByLen(
                            py_datasets.f_get_seq_len_list(), tmp_params["batch_size"]
                        )
