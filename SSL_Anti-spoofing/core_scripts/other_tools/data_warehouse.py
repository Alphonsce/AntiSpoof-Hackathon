...
class DataWarehouse:
    ...
    def _parse_file(self):
        ...
        for data_entry in data_content:
            ...
            for parse_v_method, parse_t_method in zip(
                self.parse_v_methods, self.parse_t_methods
            ):
    ...
    def get_view(self, tag_idxs, tag_values, score_parse=None):
        ...
        data_view = [
            x.get_value() for x in self.data_list if x.check_tags(tag_idxs, tag_values)
        ]
    ...
    def get_views_cross(
        self, tag_idxs, tag_values, score_parse=None, to_numpy=False, statistics=None
    ):
